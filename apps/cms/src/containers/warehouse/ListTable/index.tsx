...
import {
  warehouseListPagePaginationFetch,
  warehouseMultipleDelete,
} from "@cms/services/warehouse";
...
export const ListTable: React.FunctionComponent = () => {
  ...
  //* Query
  const fetchQueryKey = warehouseQueryKeys.list({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const { data, error } = await warehouseListPagePaginationFetch({
        query: query,
      });
  ...
  const handleDelete = async () => {
    const { error } = await warehouseMultipleDelete({
    ...
    return (
      <>
        <div className="mt-4 flex justify-between items-center gap-4">
          <div className=""></div>
          <Link to={"/warehouse/create"} className="btn btn-secondary">
            {t("create")}
          </Link>
        </div>
